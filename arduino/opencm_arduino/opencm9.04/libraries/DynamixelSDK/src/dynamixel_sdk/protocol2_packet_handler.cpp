...
int Protocol2PacketHandler::read_Rx_impl (uint8_t *rxpacket_in, PortHandler *port, uint8_t id, uint16_t length, uint8_t *data, uint8_t *error)
{
  int result                  = COMM_TX_FAIL;
 
  uint8_t *rxpacket           = rxpacket_in;
  //(length + 11 + (length/3));  // (length/3): consider stuffing
  //uint8_t *rxpacket           = new uint8_t[length + 11 + (length/3)];    // (length/3): consider stuffing
  if (!rxpacket)
  {
    rxpacket = (uint8_t *)malloc(length + 11 + (length/3));
    if (!rxpacket) return COMM_NOT_AVAILABLE;
  }
...
{
  return read_Rx_impl(NULL, port, id, length, data, error);
}

int Protocol2PacketHandler::read_TxRx_impl (uint8_t *rxpacket_in, PortHandler *port, uint8_t id, uint16_t address, 
                    uint16_t length, uint8_t *data, uint8_t *error)
{
  ...
  if (!rxpacket)
  {
    rxpacket           = (uint8_t *)malloc(11 + length + length/3);
    if (!rxpacket) return COMM_TX_FAIL;
  }
...
{
  return read_TxRx_impl(NULL, port, id, address, length, data, error);
}
...
int Protocol2PacketHandler::read1ByteRx(PortHandler *port, uint8_t id, uint8_t *data, uint8_t *error)
{
  ...
  uint8_t rxpacket[11+1];
  int result = read_Rx_impl(rxpacket, port, id, 1, data_read, error);
  if (result == COMM_SUCCESS)
    *data = data_read[0];
  return result;
}
...
int Protocol2PacketHandler::read1ByteTxRx(PortHandler *port, uint8_t id, uint16_t address, uint8_t *data, uint8_t *error)
{
  uint8_t data_read[1] = {0};
  uint8_t rxpacket[11+1];
  int result = read_TxRx_impl(rxpacket, port, id, address, 1, data_read, error);
  if (result == COMM_SUCCESS)
    *data = data_read[0];
  return result;
}
...
int Protocol2PacketHandler::read2ByteRx(PortHandler *port, uint8_t id, uint16_t *data, uint8_t *error)
{
  ...
  uint8_t rxpacket[11+2];
  int result = read_Rx_impl(rxpacket, port, id, 2, data_read, error);
  if (result == COMM_SUCCESS)
...
int Protocol2PacketHandler::read2ByteTxRx(PortHandler *port, uint8_t id, uint16_t address, uint16_t *data, uint8_t *error)
{
  uint8_t data_read[2] = {0};
  uint8_t rxpacket[11+2];
  int result = read_TxRx_impl(rxpacket, port, id, address, 2, data_read, error);
  if (result == COMM_SUCCESS)
    *data = DXL_MAKEWORD(data_read[0], data_read[1]);
  return result;
}
...
int Protocol2PacketHandler::read4ByteRx(PortHandler *port, uint8_t id, uint32_t *data, uint8_t *error)
{
  ...
  uint8_t rxpacket[11+5];  // room for stuffing
  int result = read_Rx_impl(rxpacket, port, id, 4, data_read, error);
  if (result == COMM_SUCCESS)
...
{
  uint8_t data_read[4] = {0};
  uint8_t rxpacket[11+5];   // leave enough room for stuffing. 
  int result = read_TxRx_impl(rxpacket, port, id, address, 4, data_read, error);
  if (result == COMM_SUCCESS)
    *data = DXL_MAKEDWORD(DXL_MAKEWORD(data_read[0], data_read[1]), DXL_MAKEWORD(data_read[2], data_read[3]));
  return result;
}
...
int Protocol2PacketHandler::write_Tx_TxRx_impl(uint8_t *txpacket_in, PortHandler *port, uint8_t id, 
    uint16_t address, uint16_t length, uint8_t *data, uint8_t *error, uint8_t write_type)
{
  digitalWriteFast(0, HIGH);
  //Serial.printf("p2::write_Tx_TxRx_impl packet:%x id:%d addr:%d\n", (uint32_t)txpacket_in, id, address);
  int result                  = COMM_TX_FAIL;

  uint8_t *txpacket = txpacket_in;
  uint8_t adjusted_length = length;

  // Scan through the data to see if we need to add additional room to handle ff ff fd in the data
  if (length >= 3)
  {
  ...
  if (!txpacket)
  {
    txpacket = (uint8_t *)malloc(adjusted_length + 12);
    if (!txpacket) return COMM_TX_FAIL;
  } 
  ...
  txpacket[PKT_ID]            = id;
  txpacket[PKT_LENGTH_L]      = DXL_LOBYTE(adjusted_length+5);
  txpacket[PKT_LENGTH_H]      = DXL_HIBYTE(adjusted_length+5);
  txpacket[PKT_INSTRUCTION]   = (write_type & 0x2)? INST_REG_WRITE : INST_WRITE;
  txpacket[PKT_PARAMETER0+0]  = (uint8_t)DXL_LOBYTE(address);
  txpacket[PKT_PARAMETER0+1]  = (uint8_t)DXL_HIBYTE(address);

  uint8_t *pb = &txpacket[PKT_PARAMETER0+2];
  for (uint16_t s = 0; s < length; s++)
  {
    *pb++ = data[s];
    if ((s >= 2) && (data[s-2] == 0xff) && (data[s-1] == 0xff) && (data[s] == 0xfd))
      *pb++ = 0xfd; // changed ff ff fd  to ff ff fd fd
  }

  if (write_type & 0x1) // WRITE_TXRX | INST_WRITE_TXRX
  {
    result = txRxPacket(port, txpacket, rxpacket, error);
  }
  else
...
{
  return write_Tx_TxRx_impl(NULL, port, id, address, length, data, NULL, WRITE_TX);
}
...
{
  return write_Tx_TxRx_impl(NULL, port, id, address, length, data, error, WRITE_TXRX);
}
...
int Protocol2PacketHandler::write1ByteTxOnly(PortHandler *port, uint8_t id, uint16_t address, uint8_t data)
{
  ...
  uint8_t txpacket[1+12];
  return write_Tx_TxRx_impl(txpacket, port, id, address, 1, data_write, NULL, WRITE_TX);
}
int Protocol2PacketHandler::write1ByteTxRx(PortHandler *port, uint8_t id, uint16_t address, uint8_t data, uint8_t *error)
{
  //Serial.printf("p2::write1byteTxRx id:%d addr:%d data:%d\n", id, address, data);
  uint8_t data_write[1] = { data };
  uint8_t txpacket[1+12];
  return write_Tx_TxRx_impl(txpacket, port, id, address, 1, data_write, error, WRITE_TXRX);
}
...
int Protocol2PacketHandler::write2ByteTxOnly(PortHandler *port, uint8_t id, uint16_t address, uint16_t data)
{
  ...
  uint8_t txpacket[2+12];
  return write_Tx_TxRx_impl(txpacket, port, id, address, 2, data_write, NULL, WRITE_TX);
}
int Protocol2PacketHandler::write2ByteTxRx(PortHandler *port, uint8_t id, uint16_t address, uint16_t data, uint8_t *error)
{
  uint8_t data_write[2] = { DXL_LOBYTE(data), DXL_HIBYTE(data) };
  uint8_t txpacket[2+12];
  return write_Tx_TxRx_impl(txpacket, port, id, address, 2, data_write, error, WRITE_TXRX);
}
...
int Protocol2PacketHandler::write4ByteTxOnly(PortHandler *port, uint8_t id, uint16_t address, uint32_t data)
{
  ...
  uint8_t txpacket[5+12];   // save enough room in case of FF FF FD data...  // do scan in data copy....
  return write_Tx_TxRx_impl(txpacket, port, id, address, 4, data_write, NULL, WRITE_TX);
}
int Protocol2PacketHandler::write4ByteTxRx(PortHandler *port, uint8_t id, uint16_t address, uint32_t data, uint8_t *error)
{
  uint8_t data_write[4] = { DXL_LOBYTE(DXL_LOWORD(data)), DXL_HIBYTE(DXL_LOWORD(data)), DXL_LOBYTE(DXL_HIWORD(data)), DXL_HIBYTE(DXL_HIWORD(data)) };
  uint8_t txpacket[5+12];   // save enough room in case of FF FF FD data...  // do scan in data copy....
  return write_Tx_TxRx_impl(txpacket, port, id, address, 4, data_write, error, WRITE_TXRX);
}

int Protocol2PacketHandler::regWriteTxOnly(PortHandler *port, uint8_t id, uint16_t address, uint16_t length, uint8_t *data)
{
  return write_Tx_TxRx_impl(NULL, port, id, address, length, data, NULL, REG_WRITE_TX);
}
...
{
  return write_Tx_TxRx_impl(NULL, port, id, address, length, data, error, REG_WRITE_TXRX);
}
